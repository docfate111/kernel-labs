...
static void send_test_bio(struct block_device *bdev, int dir)
{
	...
	if(submit_bio_wait(bio)){
		printk("[send_test_bio] Error submitting a bio");
	}
	printk("[send_test_bio] done submitting a bio");
	/* TODO 4: read data (first 3 bytes) from bio buffer
	and print it */
	buf = kmap_atomic(page);
	printk("[send_test_bio] first 3 bytes from bio buffer %02x %02x %02x\n", buf[0], buf[1], buf[2]);
	bio_put(bio);
	__free_page(page);
}
...
static struct block_device *open_disk(char *name)
{
	...
	bdev = blkdev_get_by_path(PHYSICAL_DISK_NAME,
			...
			THIS_MODULE);
	if(IS_ERR(bdev)){
		printk("[open_disk] blkdev_get_by_path\n");
		return NULL;
	}
	return bdev;
}
