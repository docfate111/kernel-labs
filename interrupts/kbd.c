...
static void put_char(char* c, struct kbd *data)
{
			...
			size_t size, loff_t *offset)
{
	...
	while(size--)
	{
		...
		spin_unlock_irqrestore(&data->lock, flags);
		if(!b) break;
		if(put_user(c, user_buffer++)){ return -EINVAL; }
		read++;
	}
	return read;
}
...
static int kbd_init(void)
{
	...
	struct resource* rstatus = request_region(I8042_STATUS_REG+1,
		1, MODULE_NAME);
	struct resource* rdata = request_region(I8042_DATA_REG+1,
		1, MODULE_NAME);
	if( rstatus==NULL || rdata==NULL) {
	...
	if(request_irq(I8042_KBD_IRQ, kbd_interrupt_handler,
		IRQF_SHARED, MODULE_NAME, &devs[0])<0){
		err = -EBUSY;
