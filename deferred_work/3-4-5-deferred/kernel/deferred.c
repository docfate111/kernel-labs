...
{
	printk("[work handler] handler called");
	alloc_io();
}
...
static void timer_handler(struct timer_list *tl)
{
	...
	printk("[timer_handler] current PID: %d, process name: %s", current->pid, current->comm);
	/* TODO 2: check flags: TIMER_TYPE_SET or TIMER_TYPE_ALLOC */
	switch(my_data->flag){
		case TIMER_TYPE_SET:
				break;
		case TIMER_TYPE_ALLOC:
			#ifdef ALLOC_TO_DIRECT
				alloc_io();
			#else
		...
		case TIMER_TYPE_MON:
		{
			/* TODO 4: iterate the list and check the
			 proccess state */
			struct mon_proc* i;
			struct mon_proc* tmp;
			spin_lock(&my_data->lock);
			list_for_each_entry_safe(i, tmp, &my_data->procs, list)
			{
			...
			if(i->task->state == TASK_DEAD){
				pr_info("Task %s PID %d is dead\n", i->task->comm,
					i->task->pid);
		...
		default:
				printk("[timer_handler] invalid flag");
				break;
	}
}
...
static long deferred_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case MY_IOCTL_TIMER_MON:
		{
			...
			struct mon_proc* pm = get_proc(arg);
			if (IS_ERR(pm)) return PTR_ERR(pm);
			spin_lock_bh(&my_data->lock);
			list_add(&pm->list, &my_data->procs);
...
static void deferred_exit(void)
{
	...
	list_for_each_entry_safe(p, n, &dev.procs, list){
		/* TODO 4: ... decrement task usage counter ... */
