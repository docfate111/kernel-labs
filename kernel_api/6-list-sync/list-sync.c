...
static void task_info_add_to_list(int pid)
{
	...
	printk("read lock task_info_add_to_list");
	read_lock(&lock);
	...
	printk("write lock task_info_add_to_list");
	write_lock(&lock);
...
{
	printk("starting task_info_add_for_current");
...
void task_info_print_list(const char *msg)
{
	struct list_head *p;
	struct task_info *ti;
	printk("read lock task_info_print_list");
	pr_info("%s: [ ", msg);
	/* TODO 1: Protect list, is this read or write access? */
	read_unlock(&lock);
	list_for_each(p, &head) {
...
void task_info_remove_expired(void)
{
	...
	/* TODO 1: Protect list, is this read or write access? */
	printk("write lock task_info_remove_expired");
	write_lock(&lock);
...
static void task_info_purge_list(void)
{
	struct list_head *p, *q;
	struct task_info *ti;
	printk("write lock task_info_purge_list");
	/* TODO 1: Protect list, is this read or write access? */
	write_lock(&lock);
	list_for_each_safe(p, q, &head) {
		ti = list_entry(p, struct task_info, list);
